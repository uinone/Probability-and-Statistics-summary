...
이는 $1 \over 50-X+1$ 임을 알 수 있다. 저 범위 내에서 50이 나오는 사건은 단 하나이기 때문이다.

따라서 이를 일반화 하면 다음과 같다.

$$P(B | A_i) = {1 \over 50-i+1}$$

또한 $A_1 \cdots A_{50}$ 은 서로 mutually disjoint함을 알 수 있다.

결과적으로 50개의 partitions($A_1 \cdots A_{50}$)를 가진 집합 S와 event B의 확률은 다음과 같다.

$$
P(B) = \displaystyle\sum^{50}_{i=1} P(A_i) \cdot P(B | A_i)
$$
